...
export async function getContractDeploymentTimestamp(address, scanApiDomain, scanApiKey, chainId) {
  try {
    // First get the contract creation transaction
    const sourceResponse = await fetch(mkSourceCodeUrl(address, scanApiDomain, scanApiKey, chainId));
    const sourceData = await sourceResponse.json();
    // console.log("sourceResponse", sourceResponse);
    console.log("sourceData", sourceData.result[0]);

    if (sourceData.status !== "1" || !sourceData.result[0] || !sourceData.result[0].TxHash) {
      console.error(chalk.yellow(`Could not find creation transaction for ${address}`));
      return "";
    }

    const txHash = sourceData.result[0].TxHash;

    // Get the transaction receipt to find the block number
    const txResponse = await fetch(mkTxReceiptUrl(txHash, scanApiDomain, scanApiKey, chainId));
    const txData = await txResponse.json();

    if (txData.status !== "1" || !txData.result || !txData.result.blockNumber) {
      console.error(chalk.yellow(`Could not find block number for transaction ${txHash}`));
      return "";
    }

    const blockNumber = parseInt(txData.result.blockNumber, 16);

    // Get the block info to find the timestamp
 ...
 */
export async function getSourceFilesFromAddress(address, scanApiDomain, scanApiKey, chainId, fromProxyAddress) {
  ...
  const dir = createDirWithFallback(`${name}${timestampStr}-${address}${fromProxyAddress ? `-impl-of-${fromProxyAddress}`: ""}`);
