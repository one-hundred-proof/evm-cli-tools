 ...
 *
 * Default behaviour is to download _current_ implementation contract if it is
 * a proxy contract.
 *
 * @param {string} address - Contract address
 * @param {string} scanApiDomain - API domain
...
export async function getSourceFilesFromAddress(address, scanApiDomain, scanApiKey, chainId, fromProxyAddress) {
  ...
  if (result.Implementation && result.Implementation != "" ) {
    return getSourceFilesFromAddress(result.Implementation, scanApiDomain, scanApiKey, chainId, address);
  }

  const name = path.basename(result.ContractFileName, ".sol");
  const dir = createDirWithFallback(`${name}-${address}${fromProxyAddress ? `-impl-of-${fromProxyAddress}`: ""}`);
