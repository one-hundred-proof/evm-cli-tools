...
export const CONFIG_DIR = path.join(os.homedir(), '.evm-cli-utils');
...
export function setupYargs(yargs) {
  return yargs
    ...
    .help()
    .alias('help', 'h')
    .epilogue(chalk.dim('For more information, check the documentation at https://github.com/one-hundred-proof/evm-cli-utils'))
...
export function displayChain(chainName) {
  console.log(chalk.blue(`Chain: ${chalk.bold(chainName)}`));
}
