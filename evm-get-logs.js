...
import { getCurrentChainConfig, cleanArgs } from './config-utils.js';
...
if (cleanedArgs.length < 3) {
  console.log(`Usage: ${path.basename(cleanedArgs[1])} [--chain <chain-name>] <address> [<fromBlock>] [<toBlock>]`);
  process.exit(1);
}

const address = cleanedArgs[2];
const fromBlock = cleanedArgs.length > 3 ? cleanedArgs[3] : '1';
const toBlock = cleanedArgs.length > 4 ? cleanedArgs[4] : 'latest';
